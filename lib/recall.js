...
const key = () => config.get('RECALL_API_KEY');
const base = (region) => `https://${region || config.get('RECALL_REGION')}.recall.ai/api/v1`;
const enabled = () => !!key();
...
/** 派機器人進會議 */
function deployBot(opts) { return call('POST', '/bot/', botPayload(opts)); }

/** 送給 Recall 的內容。獨立成函式，方便實測這份格式 Recall 收不收（見 scripts/verify-live.js） */
function botPayload({ meetingUrl, botName, language, joinMessage }) {
  return {
...
/** 刪除「還沒進會議」的預約機器人（已經進會議的要用 leaveCall） */
const deleteScheduledBot = id => call('DELETE', `/bot/${id}/`);
/** 實測用：用同一份內容建一個「未來才會出發」的機器人，確認 Recall 收這個格式 */
const scheduleBot = (opts, joinAt) => call('POST', '/bot/', { ...botPayload(opts), join_at: joinAt });

const getBot = id => call('GET', `/bot/${id}/`);

/** 設定精靈用：拿一組金鑰＋區域實際問 Recall 一次，確認能不能用（不會產生費用） */
async function testKey(apiKey, region) {
  const res = await fetch(`${base(region)}/bot/?page_size=1`, { headers: { Authorization: `Token ${apiKey}` } });
  if (res.ok) return { ok: true };
  if (res.status === 401 || res.status === 403) return { ok: false, reason: '金鑰不正確，或是區域選錯了。Recall 每個區域的金鑰不通用，請確認你是在哪一個區域的後台建立金鑰。' };
  return { ok: false, reason: `Recall 回應 ${res.status}，請稍後再試。` };
}
/** 不知道金鑰是哪一區的？四區都問一次，回傳第一個認得這把金鑰的區域 */
async function detectRegion(apiKey, regions) {
  const results = await Promise.all(regions.map(r => testKey(apiKey, r).then(x => (x.ok ? r : null)).catch(() => null)));
...
module.exports = { enabled, testKey, detectRegion, botPayload, scheduleBot, deleteScheduledBot, deployBot, getBot, leaveCall, mapStatus, fetchTranscript, joinWords, transcriptProvider, reasonText };
