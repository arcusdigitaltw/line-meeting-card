...
| 品牌 | 名稱、主辦單位、主色、時區、機器人的名字。都有預設值，可以先不改 |
| 對外網址 | 確認是 https 的網址（沒有的話，這一步有兩種做法的圖解） |
| 綁定 LINE | 照圖在 LINE Developers 建立 LIFF，把 LIFF ID 貼回來 |
| 會議機器人 | 貼上 Recall.ai 的 API Key，會自動找出帳號在哪一區（可跳過） |
| AI 摘要 | 貼上語言模型的金鑰，會實際呼叫一次確認能用（可跳過） |
| 最後檢查 | 每一項實際試一次；通過後一鍵建立測試會議，拿連結到手機上傳一張卡 |

![設定精靈：綁定 LINE](img/setup-4-line.png)

**成功的樣子**：最後一頁的標題變成「設定完成，可以開始用了」，每一項都是綠色。紅色的項目旁邊有「去修正」，按下去會帶你回到那一步。

![設定精靈：最後檢查](img/setup-7-check.png)

> 想了解每個 LINE 設定背後的原因，看 [02-line-liff.md](02-line-liff.md)。

## 步驟四：建一場會議，傳出去

1. 最快的做法：在精靈最後一頁按「建立測試會議，複製分享連結」，直接跳到下面第 4 點。想自己建一場的話，按「進入後台」，右上角「LIFF」標籤是綠色的才對。
