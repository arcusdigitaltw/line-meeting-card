...
2. Recall 的帳號是分區的（`us-west-2`、`us-east-1`、`eu-central-1`、`ap-northeast-1`），每一區的金鑰不通用。用設定精靈的話不用管這件事，它會自動找；只有自己寫 `.env` 才需要填 `RECALL_REGION`，填錯會一直回「金鑰不正確」。
...
1. 按「打開」到 Recall 的 API Keys 頁面（或登入後從左邊選單的 Developers → API Keys 進去）。
2. 按 **Generate API Key**，把整串金鑰複製回來貼上。
3. 按「測試金鑰」。**不用自己選區域**：精靈會四個區域都問一次，認得這把金鑰的那一區就是你的，會自動幫你選好。金鑰不對當場就知道。這個測試不會產生費用。
4. 按「存起來」。存完立刻生效，不用重啟。

![設定精靈：會議機器人](img/setup-5-recall.png)

機器人在會議裡顯示的名字預設是「品牌名・會議記錄」。想改的話，在設定精靈的「品牌」那一步改「機器人在會議裡的名字」，例如「某某公司・會議記錄」，存完立刻生效。建議寫清楚是誰家的、來做什麼，與會者看到陌生帳號進來才不會緊張。
