...
**所有需要的設定和金鑰都在精靈裡完成，不用改任何檔案；精靈走完就能用。**

1. 閱讀使用提醒
2. 用設定碼設定後台密碼
3. 品牌：名稱、主辦單位、主色、時區、機器人在會議裡的名字（有卡片預覽）
4. 確認對外網址（LINE 只接受 https；兩種取得方式都有圖解）
5. 綁定 LINE：圖解帶你在 LINE Developers 建立 LIFF，貼回 LIFF ID
6. 會議機器人：貼上 Recall.ai 的 API Key，**自動找出你的帳號在哪一區**，金鑰不對當場擋下（可跳過）
7. AI 摘要：貼上語言模型的金鑰，實際呼叫一次確認金鑰、網址、模型都對（可跳過）
8. 最後檢查：每一項都實際試一次，包含「從外面連不連得到你的網址」；有問題的項目旁邊有「去修正」。通過之後一鍵建立測試會議，拿分享連結到手機上實際傳一張卡

![設定精靈：最後檢查](docs/img/setup-7-check.png)

設定存在 `data/config.json`，存完立刻生效，不用重啟。`.env` 完全可以不用；習慣用環境變數的人（例如放在雲端平台的設定頁）也可以照 `.env.example` 設定，環境變數的優先權比精靈高。
